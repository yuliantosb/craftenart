...
<!-- End Google Tag Manager (noscript) -->
      ...
      <div class="loader">
		...
		<div class="w1">
			...
			<div class="mt-side-menu">
				@include('frontend.themes.'.config('app.themes').'.layouts.sidemenu')
			</div><!-- mt side menu end here -->
			<!-- mt search popup start here -->
			<div class="mt-search-popup">
				@include('frontend.themes.'.config('app.themes').'.layouts.search')
	...
	@if(session()->has('alert'))
		<script type="text/javascript">
			show_notification("<strong>{{ session()->get('alert.title') }}</strong>",
				"{{ session()->get('alert.type') }}", "{{ session()->get('alert.message') }}")
		</script>
	@endif
</body>
</html>